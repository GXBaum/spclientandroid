...
interface VpDao {
    ...
    @Transaction
    suspend fun clearAndInsertSelectedCourse(courses: List<VpSelectedCourseEntity>) {
        clearVpSelectedCourses()
        insertVpSelectedCourses(courses)
    }
    ...
    @Transaction
    suspend fun whateverTheFuckThisIs(
        deleteForCourseNames: List<String>,
        days: List<VpDayEntity>,
        substitutions: List<VpSubstitutionEntity>,
        infos: List<VpDayInfoItem>
    ) {
        deleteVpSubstitutionsForCourses(deleteForCourseNames)
        deleteVpDayInfo()
        clearVpDay()

        insertVpDay(days)
        insertVpSubstitutions(substitutions)
        insertVpDayInfoItems(infos)
    }
}